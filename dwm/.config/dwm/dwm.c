...
int fakesignal(void) {
  ...
  if (!strcmp(segments[0], "swalreg")) {
    ...
    if (numargs == 1) {
      winswer = strtoul(segments[1], NULL, 0);
      if ((swer = wintoclient(winswer))) swalunreg(swer);
    }
    ...
    if (numargs == 1) {
      ...
      if ((swee = wintoclient(winswee))) swalstop(swee, NULL);
    }
  }
  return 1;
}
...
void swal(Client *swer, Client *swee, int manage) {
  ...
  *pc = swee;
  ...
  if (manage) setclientstate(swee, NormalState);
  ...
  focus(NULL);
  arrange(NULL);
  if (manage) XMapWindow(dpy, swee->win);
...
void swalreg(Client *c, const char *class, const char *inst,
  ...
  s->decay = swaldecay;
  s->client = c;
  if (class) strncpy(s->class, class, sizeof(s->class) - 1);
  if (inst) strncpy(s->inst, inst, sizeof(s->inst) - 1);
...
void swalstop(Client *swee, Client *root) {
  ...
  if (!swee || !(swer = swee->swallowedby)) return;
...
void swalstopsel(const Arg *unused) {
  if (selmon->sel) swalstop(selmon->sel, NULL);
}
...
void unmanage(Client *c, int destroyed) {
  ...
  if (scratchpad_last_showed == c) scratchpad_last_showed = NULL;
  free(c);
  focus(NULL);
  updateclientlist();
  arrange(m);
